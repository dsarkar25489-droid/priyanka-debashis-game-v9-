...
// Smooth story/gameplay camera zoom.
// Far apart = zoomed out. Close together = zoomed in.
let cameraZoom =
  1.0;
...
function nextDialogue() {
  ...
  if (
    ...
    // Use the corrected left-running animation row directly.
    player.state =
      "runL";
  ...
  // Every new line begins fully visible.
...
function updateStory(
  ...
  if (
    ...
    // Priyanka runs LEFT while Debashis becomes visually farther away.
    // The camera simultaneously zooms out through updateCameraZoom().
    meeting.priyankaX =
      lerp(
        W *
        0.30,
...
function drawOpeningCharacters() {
  ...
  if (
    ...
    // Cinematic runaway: use the corrected left-facing run animation directly.
    drawCharacter(
      priyankaAtlas,
      PRIYANKA_ANIMS,
      PRIYANKA_CELL,
    ...
    if (
      ...
      ctx.font =
        `bold ${Math.round(25 * cameraZoom)}px sans-serif`;
